<x-guest-layout>
    ...
    <div class="container py-32 max-w-xl">
        <h1 class="text-4xl font-medium mb-2">{{ __('Confirm your sign up') }}</h1>
        <p class="mb-5">{{ __('For "') }}{{ $signup->maillist->display_name }}"</p>

        <p class="">{{ __('As a reminder, here are the details of this list:') }}</p>
        ...
        <form action="{{ route('signup.confirm.confirm', compact('signup')) }}" method="post">
            ...
            <x-button class="w-full justify-center font-bold">{{ __('Confirm') }}</x-button>
        </form>
