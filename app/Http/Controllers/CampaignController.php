...
class CampaignController extends Controller
{
    ...
    {
        $sendQuery = $campaign->sends()->orderByRaw('activated_at desc NULLS FIRST')->orderBy('name')->with(['mailList'])->withCount(['feeds']);
        $search = $request->get('search');
        if ($search) {
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $campaign = new Campaign(['name' => $request->get('name')]);
        $campaign->save();
        $this->showSuccessMessage(__('Campaign created!'));
    ...
    {
        $this->validate($request, ['name' => 'required|max:250']);
        $campaign->update(['name' => $request->get('name')]);
        $this->showSuccessMessage(__('Campaign updated!'));
        return redirect()->route('campaigns.show', compact('campaign'));
    }
    /**
    ...
    public function destroy(Campaign $campaign): RedirectResponse
    {
        ...
        $this->showSuccessMessage(__('Campaign deleted!'));
