...
class RssFeedController extends Controller
{
    ...
    {
        $validated = $this->validate($request, ['active' => 'required|bool', 'url' => ['required', 'url', 'max:250', new ValidRssFeedRule()], 'template_send_id' => 'required|exists:sends,id', 'send_frequency' => 'required|integer|min:1|max:366', 'target_hour' => 'required|integer|min:0|max:23']);
        $feed = new RssFeed($validated);
        $feed->last_reviewed_at = CarbonImmutable::now();
        $feed->updateNextReviewDate();
        $campaign->rssFeeds()->save($feed);
        $this->showSuccessMessage(__('RSS feed created!'));
    ...
    public function update(Request $request, Campaign $campaign, RssFeed $feed): RedirectResponse
    {
        ...
        $this->showSuccessMessage(__('RSS feed updated!'));
        return redirect()->route('campaigns.show', compact('campaign'));
    }
    /**
     * Delete the RSS feed.
    ...
    {
        $feed->delete();
        $this->showSuccessMessage(__('RSS feed deleted!'));
        return redirect()->route('campaigns.show', compact('campaign'));
    }
}