...
class ListController extends Controller
{
    ...
    public function show(MailList $list, Request $request): View
    {
        ...
        return view('lists.show', ['list' => $list, 'contacts' => $contacts]);
    }
    /**
    ...
    {
        $this->validate($request, ['name' => 'required|max:250', 'display_name' => 'required|max:250', 'slug' => 'max:250|alpha_dash|unique:mail_lists,slug']);
        $list = new MailList($request->all());
        if (empty($list->slug)) {
            $list->slug = Str::slug($list->display_name);
        }
        $list->save();
        $this->showSuccessMessage(__('List created!'));
    ...
    {
        $this->validate($request, ['name' => 'required|max:250', 'display_name' => 'required|max:250', 'slug' => "max:250|alpha_dash|unique:mail_lists,slug,{$list->id}"]);
        $list->fill($request->all());
        if (empty($list->slug)) {
            $list->slug = Str::slug($list->display_name);
        }
        $list->save();
        $this->showSuccessMessage(__('List updated!'));
        return redirect()->route('lists.show', compact('list'));
    }
    /**
    ...
    public function destroy(MailList $list): RedirectResponse
    {
        ...
        $list->delete();
        $this->showSuccessMessage(__('List deleted!'));
